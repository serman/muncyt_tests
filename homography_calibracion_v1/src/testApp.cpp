...
bool testApp::movePoint(Calibrador ccc, ofVec2f point) {
	for(int i = 0; i < ccc.ptsSrc.size(); i++) {
		if(ccc.ptsSrc[i].distance(point) < 20) {
			movingPoint = true;
			curPoint = &ccc.ptsSrc[i];
			return true;
		}
	}
	return false;
...
void testApp::mousePressed(int x, int y, int button) {
	...
	// Paso a varios calibradores, clases, etc...
	
	/// calib1
	// preparar puntos: restar offset
	vector<ofVec2f> pts_Src_C, pts_Dst_C;
	ofVec2f		cur_Src_C(x-c1_Src.x, y-c1_Src.y);
	ofVec2f		srcOffset_C(c1_Src.x, c1_Src.y);
	ofVec2f		cur_Dst_C(x-c1_Dst.x, y-c1_Dst.y);
	ofVec2f		dstOffset_C(c1_Dst.x, c1_Dst.y);
	for(int i=0; i<calib1.size(); i++) {
		pts_Src_C.push_back(calib1.ptsSrc[i] - srcOffset_C);
		pts_Dst_C.push_back(calib1.ptsDst[i] - dstOffset_C);
	}
	// Modificar punto
	if(!movePoint(calib1, cur) && !movePoint(rightPoints, cur)) {
		if(x > left.getWidth()) {
			cur -= rightOffset;
		}
		leftPoints.push_back(cur);
