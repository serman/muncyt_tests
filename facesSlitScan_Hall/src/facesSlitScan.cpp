...
void facesSlitScan::setup(){
	...
	rectHall = ofRectangle(10, 10, SCREEN_W, SCREEN_H);
...
//			warp.getDelayMap().draw(rectMap);
	...
	ofRect(rectHall);
	// divisiones
	ofSetHexColor(0x88FF0000);
	int nLinesV = 4;
	int nLinesH = 2;
	for(int i=0; i<nLinesV; i++) {
		float px = rectHall.x+rectHall.width/nLinesV * i;
		ofLine(px, rectHall.y, px, rectHall.y+rectHall.height);
	}
	for(int i=0; i<nLinesH; i++) {
		float py = rectHall.y+rectHall.height/nLinesH * i;
		ofLine(rectHall.x, py, rectHall.x+rectHall.width, py);
	}
